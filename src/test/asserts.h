...
void assert_eq(T expected, T value) {
	...
	if (std::is_floating_point<T>::value) {
		...
		if (eps > EPSILON) {
			c << "Expected " << std::fixed << std::setprecision(11) << a
					<< " and got " << std::fixed << std::setprecision(11) << b;
...
void assert_neq(T expected, T value) {
	if (expected == value) {
		...
		if (std::is_floating_point<T>::value)
			c << "Expected " << std::fixed << std::setprecision(3) << expected
					<< " and got " << std::fixed << std::setprecision(3)
					<< value;
		else
			c << "Expected " << expected << " and got " << value;
...
void assert_vector_eq(T expected[], T value[], unsigned long size) {
	...
	if (expected_stream.str().compare(value_stream.str()) != 0) {
		c << "Expected "
				<< expected_stream.str().substr(0,
						expected_stream.str().size() - 2) << " and got "
				<< value_stream.str().substr(0, value_stream.str().size() - 2);
